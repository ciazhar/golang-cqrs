...
func (b *AmqpBroker) setup() error {
	logger.Info("Setup AMQP Connection")
	if b.conn != nil {
		return nil
	}
	...
	conn, err := b.connect()
	if err != nil {
		//return log.WithError("connection failed: ",err)
		return err
	}

	err = b.Init(conn)
	if err != nil {
		//return log.WithError("client init failed: %v",err)
		return err
	}
...
func (b *AmqpBroker) watch() {
	for {
		select {
		case err := <-b.errors:
			logger.Warn("Connection lost: ", err)
			b.Lock()
			b.conn = nil
			b.Unlock()
