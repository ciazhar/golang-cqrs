...
func (pub *amqpPublisherManager) CreatePublisher(publishKey string) (common.Publisher, error) {
	logger.Infof().Str("publishKey", publishKey).Msgf("create publisher")
...
func (client *amqpPublisher) init(connection *amqp.Connection) error {
	logger.Info("Amqp publisher initiated %s", client.publishKey)
